...
plot_figure1 <- function(agestructure, btsp, location=1, seroposdates, nplots=2){
  dataP=NULL
  locnnLoop=2
  ...
  for(iiH in location){ # NOTE CURRENTLY JUST 2013/14
    ...
    medInf=apply(ivector,2,function(x){median(x)})
  ...
  dev.copy(pdf,paste("post_plotsZ/Figure_modelfit_cases",iiH,".pdf",sep=""),width=6,height=6)#,height=8)
  dev.off()
  } #end location loop
}# end function
