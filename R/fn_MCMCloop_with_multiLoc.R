...
MCMCloop_with_multiLoc <- function(agestructure, sample.start.point=T, all.Priors=F, include.count=T, startdate=as.Date("2013-10-28")){
  for (m in 1:MCMC.runs){
    ...
    for(kk in itertab){ 
      iiH=kk
      #print(locationtab[iiH])
      data <- load.data.multiloc.multistart(agestructure, add.nulls = 0, startdate, virusTab[1], dataTab[1], serology.excel, init.conditions.excel)
        list2env(data,globalenv())
      y.vals <- y.vals.df[,locationtab[iiH]]
      #date.interventions <- vector_control_dates[,locationtab[iiH]]
      #date.interventions <- date.interventions[date.interventions != ""]
      #time.interventions <- apply(as.matrix(date.interventions), 1, function(x){min(which(x))})
      time.interventions <- v.c.vals.df[,locationtab[iiH]]
      index.interventions=which(time.interventions!=0)
      #print(time.interventions)
      ...
      if(sample.start.point==T){
        ...
        if(Sample.StartTime>0){
          ...
          data <- load.data.multiloc.multistart(agestructure, add.nulls=0, new.start.time, virusTab[1], dataTab[1], serology.excel, init.conditions.excel)
        ...
        data <- load.data.multiloc.multistart(agestructure,add.nulls=0, startdate, virusTab[iiH], dataTab[1], serology.excel, init.conditions.excel)
      }
        list2env(data, globalenv())
      y.vals <- y.vals.df[,locationtab[iiH]]
