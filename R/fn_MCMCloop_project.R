...
#' @param project.fwd.wks number of weeks to project forward
...
##' @export 
#parameter_est_file="parameters_est"
#project.fwd.wks=52
MCMCloop_project <- function(agestructure, project.fwd.wks, sample.start.point=T, all.Priors=F, include.count=T, startdate=as.Date("2013-10-28")){

# Storage vector for forward sim:
  ...
  max.length.sim = length(time.vals)+project.fwd.wks
  time.vals.project.fwd=c(time.vals, seq(time.vals[length(time.vals)], time.vals[length(time.vals)]+(project.fwd.wks*7), 7))
  c_trace_tab_sim=array(NA, dim=c(MCMC.runs+1,locnn,max.length.sim)) 
  ...
  for (m in 1:MCMC.runs){
    ...
    #run model with parameter selected values for a longer period
    output2=Deterministic_modelR_project(agestructure,time.vals.project.fwd,c(thetatab[m+1,],thetaAlltab[m+1,1,]), theta_initAlltab[m+1,1,], locationI=locationtab[iiH], seroposdates=seroposdates, episeason=episeason, include.count=include.count)
        #plot(output2$C_trace)
    #length(output2$C_trace)
    #length(c_trace_tab_sim[m+1,1,])
    #max.length.sim = length(time.vals)+project.fwd.wks
    c_trace_tab_sim[m+1,1,] <- output2$C_trace[1:max.length.sim]
  ...
  if(agestructure==1){
    ...
    return(list(sim_liktab=sim_liktab,
                ...
                accepttab=accepttab,
                c_trace_tab=c_trace_tab,
                c_trace_tab_sim=c_trace_tab_sim,
                s_trace_tab=s_trace_tab,
                r_trace_tab=r_trace_tab,
                x_trace_tab=x_trace_tab,
                thetatab=thetatab,
