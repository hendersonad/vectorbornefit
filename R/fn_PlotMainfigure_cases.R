...
plot_figure1_cases_v1 <- function(agestructure, btsp, location=1, seroposdates, nplots=2, sample.start.point=T, startdate=as.Date("2013-10-28")){
  require(gridExtra)
  ...
  for(iiH in location){ # NOTE CURRENTLY JUST 2013/14
    ...
    dataframe.p2 <- data.frame(date.vals=date.vals[1:length(y.vals.plot)], y.vals.plot, medP, ciP1, ciP2, ciP150, ciP250)
    ...
    plotCosR0 = NULL; plotCosRR = NULL; plotDecline=NULL; plotSeas=NULL; plotBeta=NULL
    ...
    for(ii in 1:length(btstrap)){
      ...
      beta_jj = decline_f(time.V, date0, mask=thetatab[b_ii,'beta_mask'], base=thetatab[b_ii,'beta_base'], grad=thetatab[b_ii,'beta_grad'], mid=thetatab[b_ii,'beta_mid'])
      ...
      output_rr = calculate_r0(th_in=thetatab[b_ii,],sus_c=s_pick,sus_a=0,sm_c=x_pick,sm_a=0,b_vary=beta_ii*beta_jj)
      ...
      plotDecline=rbind(plotDecline,  beta_jj)
      plotSeas=rbind(plotSeas,  beta_ii)
      plotBeta=rbind(plotBeta,  thetatab[b_ii,'beta_h']*beta_ii*beta_jj)
    }
      c.nume<-function(x){
      ...
      plotDecM  = apply(plotDecline,2,c.nume)
      plotSeasM  = apply(plotSeas,2,c.nume)
      plotBetaM  = apply(plotBeta,2,c.nume)
    ...
    dataframe.p5 <- data.frame(date.vals, 
                               medR0=plotCosMR0[1,], lciR0=plotCosMR0[2,], uciR0=plotCosMR0[3,],
                               medRR=plotCosMRR[1,], lciRR=plotCosMRR[2,], uciRR=plotCosMRR[3,],
                               medD=plotDecM[1,], lciD=plotDecM[2,], uciD=plotDecM[3,],
                               medS=plotSeasM[1,], lciS=plotSeasM[2,], uciS=plotSeasM[3,],
                               medBeta=plotBetaM[1,], lciBeta=plotBetaM[2,], uciBeta=plotBetaM[3,])
    ...
    (p5 <- ggplot(dataframe.p5) + 
        geom_ribbon(aes(x=date.vals, ymin=lciS, ymax=uciS),col=col1a, fill=col1a) +
        geom_line(aes(x=date.vals, y=medS), col=col1, linetype=1) +
        geom_ribbon(aes(x=date.vals, ymin=lciD, ymax=uciD),col=col2a, fill=col2a) +
        geom_line(aes(x=date.vals, y=medD), col=col2, linetype=1) +
        geom_line(aes(x=date.vals, y=1), col=1, linetype=2) +
        labs(x = "Year", y = "RR", title = (LETTERS[labelN])) +
        scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
        scale_y_continuous(limits=c(0,max(dataframe.p5$uciS))) +
        theme_zika_fiji())
