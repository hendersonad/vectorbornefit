...
NOLIKELIHOOD_AdaptMCMCloop_withDENVimmunity <- function(sample.start.point=T, startdate=as.Date("2013-10-28"), 
                                      adapt.shape.start=MCMC.runs){
  for (m in 1:MCMC.runs){
    ...
    for(kk in itertab){ 
      ...
      prior.star <- 1#(prior.theta$prior.star*prior.star)
      prior.current <- 1#(prior.theta$prior*prior.current)
    ...
    val = (prior.star/prior.current) + exp((q_theta_given_theta_star - q_theta_star_given_theta)) 
    ...
    if(m %% thinning.parameter == 0){
      ...
      if(m>1 & MH_random_unif < output_prob){
