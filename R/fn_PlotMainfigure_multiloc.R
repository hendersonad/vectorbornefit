...
plot_figure1_multiloc <- function(agestructure, btsp, location=1, seroposdates, nplots=2, sample.start.point=T, startdate=as.Date("2013-10-28")){
  require(gridExtra)
  ...
  for(iiH in itertab){ # NOTE CURRENTLY JUST 2013/14
    ...
    if(agestructure==0){
      for(date in seroposdates){
        binomtest <- binom.test(x=nLUM[iiH,i],n=nPOP[iiH,i])
        lci[i] <- binomtest$conf.int[1]
        uci[i] <- binomtest$conf.int[2]
        points[i] <- nLUM[iiH,i]/nPOP[iiH,i]
        i <- i+1
      }
    dataframe.sero <- data.frame(points,lci,uci, seroposdates)
    }else if(agestructure==1){
      for(date in seroposdates){
        binomtestC <- binom.test(x=nLUM[iiH,i]+nLUM,n=nPOP[iiH,i]) 
        binomtestA <- binom.test(x=nLUM[iiH,(length(nLUM)/2)+1], n=nPOP[iiH,(length(nLUM)/2)+1])
        ...
        points_A[i] <- (nLUM[iiH,(length(nLUM)/2)+1]/nPOP[iiH,(length(nLUM)/2)+1])
        i <- i+1
      }
